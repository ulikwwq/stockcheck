...
public interface SaleItemRepository extends JpaRepository<SaleItem, UUID> {
     ...
     * [0] day (java.sql.Date), [1] revenue (only known-price items),
     * [2] cost (only known-cost items), [3] profit (only fully-known
     * items), [4] count of items where price and/or cost is unknown
     * (profit not calculable for those units).
     */
    @Query(value = "SELECT CAST(s.created_at AS date) AS day, " +
            "COALESCE(SUM(CASE WHEN si.sale_price IS NOT NULL THEN si.sale_price * si.quantity ELSE 0 END), 0) AS revenue, " +
            "COALESCE(SUM(CASE WHEN si.purchase_price IS NOT NULL THEN si.purchase_price * si.quantity ELSE 0 END), 0) AS cost, " +
            "COALESCE(SUM(si.profit), 0) AS profit, " +
            "SUM(CASE WHEN si.purchase_price IS NULL OR si.sale_price IS NULL THEN si.quantity ELSE 0 END) AS unknown_cost_units " +
            "FROM sale_items si " +
